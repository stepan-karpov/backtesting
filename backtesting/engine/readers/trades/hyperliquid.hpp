...
class TradesFileReader {
public:
    explicit TradesFileReader(const std::string& path) {
        ...
        stream_ = std::istringstream(
            std::string(decompressed.data(), bytes_written)
        );
    }
...
// ── Агрегирует несколько TradesFileReader-ов, выдаёт строки по порядку ────────

class TradesReader {
public:
    explicit TradesReader(std::vector<std::string> file_paths)
        : file_paths_(std::move(file_paths))
        , file_index_(0)
    {
        if (!file_paths_.empty()) {
            current_ = std::make_unique<TradesFileReader>(file_paths_[file_index_]);
        }
    }
...
private:
    ...
    size_t                             file_index_;
    std::unique_ptr<TradesFileReader>  current_;
};